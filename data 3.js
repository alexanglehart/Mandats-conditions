...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  interestTab.textContent='🔵 Intérêt 0';
  ...
  function updateCategoryLabels(){
    ...
    const m=document.getElementById('mandatsTab');
    const c=document.getElementById('conditionsTab');
    if(m)m.textContent='🔴 Mandats '+counts.mandat;
    if(c)c.textContent='🟠 Conditions '+counts.condition;
    interestTab.textContent='🔵 Intérêt '+counts.interest;
  }
  ...
  openDetail=function(id){
    ...
    if(complete&&p.type==='interest')complete.textContent='✓ Personne d’intérêt traitée';
  ...
  completePerson=function(id){
    ...
    if(p?.type==='interest'){
      ...
      p.status='done';save();closeDetail();render();updateCategoryLabels();return;
    }
    originalCompletePerson(id);
    setTimeout(updateCategoryLabels,0);
  };
